...
public class ServiceUser {
    ...
    public ModelNguoiDung login(ModelLogin login) throws SQLException {
        ...
        String sql = "SELECT ID_ND, Email, Matkhau, Vaitro FROM NguoiDung "
                + "WHERE Email = ? AND Matkhau = ? AND Trangthai = 'Verified' "
                + "FETCH FIRST 1 ROWS ONLY"; // Cú pháp chuẩn cho Oracle 12c+
    ...
    public void insertUser(ModelNguoiDung user) throws SQLException {
        if (user == null) {
            throw new NullPointerException("User không được rỗng");
        }
    ...
    public boolean checkDuplicateCode(String code) throws SQLException {
        if (code == null || code.trim().isEmpty()) {
            throw new IllegalArgumentException("Mã xác minh không được rỗng");
        }
